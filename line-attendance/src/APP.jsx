...
// Firebaseコンソール → プロジェクト設定 → マイアプリ → CDN の firebaseConfig
// ──────────────────────────────────────────────────────────
...
function serializeEvent(ev) {
  return {
    ...
    attendees: ev.attendees
      ? ev.attendees.map(a => ({ ...a, time: a.time instanceof Date ? a.time.toISOString() : a.time }))
      : [],
  };
}
// Firebaseから取得したデータを復元
...
export default function App() {
  ...
  const [respondForm,  setRespondForm]  = useState({ name: "", status: "参加" });
  ...
  useEffect(() => {
    ...
    const unsub = onValue(eventsRef, (snapshot) => {
      ...
      if (data) {
        const list = Object.entries(data).map(([fbKey, val]) => ({
          ...deserializeEvent(val),
          fbKey,
        }));
        list.sort((a,b) => new Date(a.date) - new Date(b.date));
        setEvents(list);
        // 現在開いているイベントも最新データに更新
  ...
  const openEvent = (ev) => { setSelectedEvent(ev); setScreen("event"); };

  // 🔥 出欠回答をFirebaseに保存
  const submitResponse = async () => {
    if (!respondForm.name.trim() || !selectedEvent) return;
    const entry = {
      id:     Date.now(),
      name:   respondForm.name.trim(),
      status: respondForm.status,
      time:   new Date().toISOString(),
      isNew:  true,
    };
    const attendeesRef = ref(db, `events/${selectedEvent.fbKey}/attendees`);
    await push(attendeesRef, entry);
    setNewHighlight(entry.id);
    setTimeout(() => setNewHighlight(null), 4000);
    setRespondForm({ name: "", status: "参加" });
    showToast("✅ 回答しました！");
    setScreen("event");
  };
  ...
  const submitNewEvent = async () => {
    if (!editForm.title.trim()) return;
    const eventsRef = ref(db, "events");
    await push(eventsRef, serializeEvent({ ...editForm, id: Date.now(), attendees: [] }));
    ...
    <div style={S.shell}>
      <div style={S.phone}>
        <div style={S.loadingScreen}>
          ...
          <div style={S.loadingText}>データを読み込み中...</div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={S.shell}>
      <div style={S.phone}>
        <div style={S.statusBar}><span>9:41</span><span style={{letterSpacing:2}}>●●●</span></div>

        {screen === "calendar" && (
          <CalendarScreen events={events} calYear={calYear} calMonth={calMonth}
            setCalYear={setCalYear} setCalMonth={setCalMonth}
            selectedDate={selectedDate} setSelectedDate={setSelectedDate}
            onOpenEvent={openEvent} onNewEvent={openNewEvent} />
        )}
        {screen === "event" && selectedEvent && (
          <EventScreen event={selectedEvent} onBack={() => setScreen("calendar")}
            onRespond={() => setScreen("respond")} onEdit={() => openEdit(selectedEvent)}
            onDelete={() => deleteEvent(selectedEvent.fbKey)}
            newHighlight={newHighlight} />
        )}
        {screen === "respond" && selectedEvent && (
          <RespondScreen event={selectedEvent} form={respondForm} setForm={setRespondForm}
        ...
        <div style={S.bottomNav}>
          ...
          <NavBtn icon="🔔" label="通知" active={false} onClick={() => showToast("通知機能はLIFF連携後に有効になります")} />
        </div>
            ...
            {week.map((d,di) => {
              ...
              return (
                <div key={di} style={{...S.calCell, opacity:d?1:0.2}} onClick={() => d && setSelectedDate(d)}>
                  <div style={{...S.calNum, background:isToday?"#00B900":isSelected?"#e6f9e6":"transparent",
                    color:isToday?"#fff":di===0?"#EF4444":di===6?"#3B82F6":"#333",
                    fontWeight:isToday||isSelected?700:400}}>{d||""}</div>
                  <div style={S.calDots}>{evs.slice(0,3).map(ev=><div key={ev.fbKey||ev.id} style={{...S.calDot,background:ev.color}}/>)}</div>
                </div>
              );
            })}
          </div>
        ...
        {selectedDate ? (
          <>
            ...
            {selectedEvents.length===0
              ...
              : selectedEvents.map(ev => <EventCard key={ev.fbKey||ev.id} ev={ev} onClick={() => onOpenEvent(ev)} />)
            }
          </>
        ) : (
          <div style={S.allEventList}>
            ...
            {events.filter(e => new Date(e.date) >= new Date(TODAY.getFullYear(),TODAY.getMonth(),TODAY.getDate()))
              .map(ev => <EventCard key={ev.fbKey||ev.id} ev={ev} onClick={() => onOpenEvent(ev)} />)}
          </div>
        )}
      </div>
    </div>
...
function EventCard({ ev, onClick }) {
  ...
  return (
    <div style={{...S.eventCard, borderLeft:`4px solid ${ev.color}`}} onClick={onClick}>
      ...
      <span style={{...S.eventCardBadge, background:ev.color}}>{attending}/{ev.capacity}名</span>
    </div>
  );
}

// ─── Event Detail Screen ───────────────────────────────────
function EventScreen({ event, onBack, onRespond, onEdit, onDelete, newHighlight }) {
  ...
  const remaining = event.capacity - attending.length;
  ...
  return (
    <div style={S.screen}>
      ...
      <div style={S.capacityStrip}>
        <div style={S.capBar}>
          <div style={{...S.capFill, width:`${Math.min(100,(attending.length/event.capacity)*100)}%`, background:remaining<=3?"#EF4444":event.color}}/>
        </div>
        <div style={S.capNums}>
          <span style={{color:event.color,fontWeight:700}}>{attending.length}名参加</span>
          <span style={{color:"#aaa"}}> / {event.capacity}名定員</span>
          {remaining>0
            ? <span style={{marginLeft:"auto",color:remaining<=5?"#EF4444":"#666",fontWeight:600}}>残{remaining}席</span>
            : <span style={{marginLeft:"auto",color:"#EF4444",fontWeight:700}}>満席</span>}
        </div>
      </div>
      ...
      <div style={S.tabContent}>
        {tab==="info" && (
          <div style={{padding:"12px 16px"}}>
            ...
            <InfoRow icon="👥" label="定員" value={`${event.capacity}名`} />
            <InfoRow icon="⏰" label="締切" value={fmt(event.deadline)} />
            {event.note && <InfoRow icon="📝" label="備考" value={event.note} />}
            <div style={S.summaryCards}>
              {[["参加",attending.length,"#00B900"],["欠席",absent.length,"#EF4444"],["未定",undecided.length,"#9CA3AF"]].map(([label,count,color])=>(
                <div key={label} style={S.summaryCard}>
                  <div style={{...S.summaryCount,color}}>{count}</div>
        ...
        {tab==="list" && (
          <AttendeeList attendees={event.attendees} capacity={event.capacity} newHighlight={newHighlight} color={event.color} />
        )}
      </div>
    </div>
  );
...
function AttendeeList({ attendees, capacity, newHighlight, color }) {
  const sorted = [...attendees].sort((a,b)=>new Date(a.time)-new Date(b.time));
  const attending = sorted.filter(a=>a.status==="参加");
  return (
    <div style={{padding:"8px 12px"}}>
      <div style={S.listSectionLabel}>回答一覧（先着順）</div>
      {sorted.length===0 && <div style={S.calEmpty}>まだ回答がありません</div>}
      {sorted.map((a,i) => {
        const rank = attending.indexOf(a);
        const isOver = a.status==="参加" && rank>=capacity;
        const isHL = a.id===newHighlight;
        return (
          <div key={a.id} style={{...S.attendeeRow,background:isHL?"#fffde7":"#fff",border:isHL?`2px solid ${color}`:"1px solid #f0f0f0"}}>
            <div style={S.orderCircle(i,color)}>{i+1}</div>
            <div style={{...S.aAvatar,background:STATUS_META[a.status].bg}}>{a.name[0]}</div>
            <div style={S.aInfo}>
              <div style={S.aName}>{a.name}{a.isNew&&<span style={S.newBadge}>NEW</span>}</div>
              <div style={S.aTime}>🕐 {fmtFull(new Date(a.time))}</div>
            </div>
            <div style={S.aPill(a.status)}>{a.status==="参加"&&isOver?"補欠":a.status}</div>
            {a.status==="参加"&&!isOver&&(
              <div style={{fontSize:rank<3?18:11,marginLeft:2}}>{rank===0?"🥇":rank===1?"🥈":rank===2?"🥉":`${rank+1}番`}</div>
            )}
          </div>
        );
      })}
...
// ─── Respond Screen ────────────────────────────────────────
function RespondScreen({ event, form, setForm, onSubmit, onBack }) {
  const remaining = event.capacity - event.attendees.filter(a=>a.status==="参加").length;
  return (
    <div style={S.screen}>
      <div style={{...S.eventDetailHeader,background:event.color,paddingBottom:14}}>
        <div style={S.eventHeaderTop}>
          <button style={S.backBtn} onClick={onBack}>‹</button>
          <div style={S.eventHeaderTitle}>出欠を回答する</div>
          <div style={{width:32}}/>
        </div>
        <div style={S.eventHeaderMeta}>{event.title}</div>
      </div>
      <div style={{padding:"16px"}}>
        <label style={S.formLabel}>お名前</label>
        <input style={S.formInput} placeholder="例：山田 太郎" value={form.name} onChange={e=>setForm({...form,name:e.target.value})} />
        <label style={S.formLabel}>出欠</label>
        <div style={S.statusBtns}>
          {["参加","欠席","未定"].map(s=>(
            <button key={s} style={{...S.statusBtn,background:form.status===s?STATUS_META[s].bg:"#f3f4f6",color:form.status===s?"#fff":"#555",border:`2px solid ${form.status===s?STATUS_META[s].bg:"#e5e7eb"}`}} onClick={()=>setForm({...form,status:s})}>
              {STATUS_META[s].icon} {s}
            </button>
          ))}
        </div>
        {form.status==="参加"&&remaining<=5&&remaining>0&&<div style={S.warnBox}>⚠️ 残席わずか！（残{remaining}席）</div>}
        {form.status==="参加"&&remaining<=0&&<div style={{...S.warnBox,background:"#fee2e2",borderColor:"#EF4444"}}>😢 定員に達しています。補欠登録になります。</div>}
        <button style={{...S.respondBtn,background:form.name.trim()?event.color:"#ccc",marginTop:24}} onClick={onSubmit} disabled={!form.name.trim()}>送信する</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={S.screen}>
      ...
      <div style={{overflowY:"auto",flex:1,padding:"14px 16px",display:"flex",flexDirection:"column",gap:12}}>
        ...
        <Field label="定員（人）">
          <input type="number" style={S.formInput} value={form.capacity} onChange={e=>setForm({...form,capacity:parseInt(e.target.value)||1})} min={1} />
        </Field>
        <Field label="回答締切">
...
const S = {
  ...
  statusBar:{ background:"#00B900", color:"#fff", fontSize:11, padding:"6px 20px", display:"flex", justifyContent:"space-between" },
  ...
  loadingIcon:{ fontSize:40, animation:"spin 1s linear infinite" },
  ...
  eventCardTitle:{ fontWeight:700, fontSize:14, color:"#111", marginBottom:3 },
  eventCardMeta:{ fontSize:11, color:"#888" },
  eventCardBadge:{ color:"#fff", borderRadius:10, padding:"3px 8px", fontSize:11, fontWeight:700 },
  ...
  capBar:{ height:8, background:"#e5e7eb", borderRadius:4, overflow:"hidden", marginBottom:6 },
  ...
  infoRowLabel:{ fontSize:12, color:"#888", width:40 },
  infoRowValue:{ fontSize:13, color:"#333", flex:1, lineHeight:1.5 },
  summaryCards:{ display:"flex", gap:8, margin:"16px 0" },
  ...
  listSectionLabel:{ fontSize:12, fontWeight:700, color:"#888", marginBottom:8, letterSpacing:0.5 },
  ...
  aAvatar:{ width:34, height:34, borderRadius:"50%", flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontWeight:700, fontSize:13 },
  aInfo:{ flex:1, minWidth:0 },
  aName:{ fontSize:13, fontWeight:600, color:"#111", display:"flex", alignItems:"center", gap:4 },
  aTime:{ fontSize:10, color:"#aaa", marginTop:2 },
  ...
  bottomNav:{ background:"#fff", borderTop:"1px solid #e5e7eb", display:"flex", padding:"6px 0 4px" },
